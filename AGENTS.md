...
- Local build/test helpers live in `tools/`: `Pack-GloryKills.ps1` (Windows, rebuilds the load-folder zip). `smoke-test-macos.sh` (macOS headless compile + MAP01 run) also lives there but is gitignored — it's a machine-specific local harness, not tracked in the remote repo. Neither is shipped content
